...
const EarlyAccessPage = () => {
    ...
    // New State for Selection and Filters
    ...
    const exportCSV = () => {
        ...
        const csvContent =
            ...
            ["Name,Email,Phone,Interest,Status,Date"]
                .concat(
                    rowsToExport.map(
                        (row) =>
                            `"${row.name || ""}","${row.email}","${row.phone || ""}","${(row.interest || "").replace(/"/g, '""')}","${row.status
        ...
        link.setAttribute("download", `early_access_${selectedIds.size > 0 ? 'selected' : 'all'}.csv`);
    ...
    return (
        <div className="space-y-6">
            <div className="sm:flex sm:items-center sm:justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Early Access</h1>
                    ...
                    <tbody className="divide-y divide-slate-200 bg-white">
                        {loading ? (
                            ...
                            filteredData.map((person) => (
                                <tr key={person.id} className={selectedIds.has(person.id) ? "bg-slate-50" : ""}>
                                    ...
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-900">
                                        ...
                                        <div className="text-slate-500 text-xs">{person.name || "-"}</div>
                                    </td>
                                    ...
                                    <td className="whitespace-nowrap px-3 py-4 text-sm">
                                        <select
                                            value={person.status}
                                            onChange={(e) =>
                                                handleStatusUpdate(person.id, e.target.value)
                                            }
                                            className={`rounded-md border-0 py-1 pl-2 pr-8 text-xs font-medium ring-1 ring-inset ${person.status === "New"
